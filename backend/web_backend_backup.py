...
def generate_music():
    ...
    try:
        ...
        output_path = generate_instrumental(prompt, duration=duration)
...
def generate_codebeat_pattern():
    ...
    try:
        ...
        # Use MusicGen backend for real audio generation
        from musicgen_backend import generate_instrumental
        
        # Generate the audio file
        output_path = generate_instrumental(enhanced_prompt, duration=120)
        ...
        return jsonify({
            "jobId": job_id,
            "status": "finished",
            "message": "Code pattern converted to music!",
            "tempo": tempo,
            "key": key
        }), 200
...
def generate_music():
    ...
    data = request.json or {}
    prompt = data.get('prompt', '')
    duration = min(int(data.get('duration', 30)), 120)  # Cap duration at 2 minutes
    ...
    try:
        ...
        return send_file(
            ...
            mimetype='audio/wav',
            as_attachment=True,
            download_name=f"generated_music_{int(time.time())}.wav"
        )
        
    except Exception as e:
        logging.error(f"Error generating music: {str(e)}", exc_info=True)
